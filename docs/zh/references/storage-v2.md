...
- Renderer 已加入普通对话 / 文件显式删除 tombstone：删除 topic 或文件时会写入 Storage v2 软删除，避免恢复后复活；单个 topic 删除、清空助手全部 topic、删除助手这类破坏性入口会先等待 legacy runtime 删除和 Storage v2 tombstone 路径完成，再更新助手列表 Redux runtime cache，`assistants` reducer 不再直接触发异步删除副作用，降低 UI 已删除但权威库尚未落盘的崩溃窗口。
