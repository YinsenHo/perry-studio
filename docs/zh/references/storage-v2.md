...
- assistant 新增、复制、插入、删除、排序、编辑、默认助手更新、模型和 settings 更新后会主动 flush Redux -> Storage v2 mirror；assistant topic 消息仍由普通对话 mirror 负责，核心 assistant 元数据不会只依赖低频防抖写入。provider 和 assistant 删除会先走 Storage v2 direct tombstone IPC，成功后才更新 Redux runtime cache 并 strict flush；assistant topic、assistant preset 和 MCP server 删除会等待 Redux mirror strict flush 成功，避免本地配置和敏感连接信息在恢复时复活。
